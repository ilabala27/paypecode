...
const { baseURL, header, log, timeout } = appConfig.apiConnections.paype.creditWallet
import { Auth } from "aws-amplify";

export const CreditWalletAxios = DefaultAxios.create({
    baseURL,
    timeout: timeout
});
...
const requestHandler = async (request: any) => {
    ...
    if (log) console.log("### request from PAYPE-CREDIT_WALLET", request)
    return request;
}
...
const requestInterceptor = CreditWalletAxios.interceptors.request.use(
    (request) => requestHandler(request),
    (error) => errorHandler("onRequest-CREDIT_WALLET-interceptor", error)
);

// onResponse Interceptor
const responseInterceptor = CreditWalletAxios.interceptors.response.use(
    (response) => responseHandler(response),
    (error) => errorHandler("onResponse-CREDIT_WALLET-interceptor", error)
);
