...
const { baseURL, header, log, timeout } = appConfig.apiConnections.paype.tradeWallet
import { Auth } from "aws-amplify";

export const TradeWalletAxios = DefaultAxios.create({
    baseURL,
    timeout: timeout
});
...
const requestHandler = async (request: any) => {
    ...
    if (log) console.log("### request from PAYPE-TRADE_WALLET", request)
    return request;
}
...
const requestInterceptor = TradeWalletAxios.interceptors.request.use(
    (request) => requestHandler(request),
    (error) => errorHandler("onRequest-TRADE_WALLET-interceptor", error)
);

// onResponse Interceptor
const responseInterceptor = TradeWalletAxios.interceptors.response.use(
    (response) => responseHandler(response),
    (error) => errorHandler("onResponse-TRADE_WALLET-interceptor", error)
);
