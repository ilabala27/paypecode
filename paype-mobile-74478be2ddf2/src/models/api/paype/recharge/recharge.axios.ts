...
const { baseURL, header, log, timeout } = appConfig.apiConnections.paype.recharge
import { Auth } from "aws-amplify";

export const RechargeAxios = DefaultAxios.create({
    baseURL,
    timeout: timeout
});
...
const requestHandler = async (request: any) => {
    ...
    if (log) console.log("### request from PAYPE-SERVICES", request)
    return request;
}
...
const requestInterceptor = RechargeAxios.interceptors.request.use(
    (request) => requestHandler(request),
    (error) => errorHandler("onRequest-PAYPE-SERVICES-interceptor", error)
);

// onResponse Interceptor
const responseInterceptor = RechargeAxios.interceptors.response.use(
    (response) => responseHandler(response),
    (error) => errorHandler("onResponse-PAYPE-SERVICES-interceptor", error)
);
