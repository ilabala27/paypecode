...
const { baseURL, header, log, timeout } = appConfig.apiConnections.paype.user
import { Auth } from "aws-amplify";
import { getIdToken, getToken } from "../utils.axios";

export const UserAxios = DefaultAxios.create({
    baseURL,
    timeout: timeout
});
...
const requestHandler = async (request: any) => {
    ...
    if (log) console.log("### request from PAYPE-USER", request)
    return request;
}
...
const requestInterceptor = UserAxios.interceptors.request.use(
    ...
    (error) => errorHandler("onRequest-USER-interceptor", error)
);

// onResponse Interceptor
const responseInterceptor = UserAxios.interceptors.response.use(
    (response) => responseHandler(response),
    (error) => errorHandler("onResponse-USER-interceptor", error)
);
