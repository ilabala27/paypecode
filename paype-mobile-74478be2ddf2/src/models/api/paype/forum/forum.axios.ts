...
const { baseURL, header, log, timeout } = appConfig.apiConnections.paype.forum
import { Auth } from "aws-amplify";

export const ForumAxios = DefaultAxios.create({
    baseURL,
    timeout: timeout
});
...
const requestHandler = async (request: any) => {
    ...
    if (log) console.log("### request from PAYPE-Forum", request)
    return request;
}
...
const requestInterceptor = ForumAxios.interceptors.request.use(
    (request) => requestHandler(request),
    (error) => errorHandler("onRequest-Forum-interceptor", error)
);

// onResponse Interceptor
const responseInterceptor = ForumAxios.interceptors.response.use(
    (response) => responseHandler(response),
    (error) => errorHandler("onResponse-Forum-interceptor", error)
);
