...
const { baseURL, header, log, timeout } = appConfig.apiConnections.paype.services
import { Auth } from "aws-amplify";

export const ServicesAxios = DefaultAxios.create({
    baseURL,
    timeout: timeout
});
...
const requestInterceptor = ServicesAxios.interceptors.request.use(
    (request) => requestHandler(request),
    (error) => errorHandler("onRequest-PAYPE-SERVICES-interceptor", error)
);

// onResponse Interceptor
const responseInterceptor = ServicesAxios.interceptors.response.use(
