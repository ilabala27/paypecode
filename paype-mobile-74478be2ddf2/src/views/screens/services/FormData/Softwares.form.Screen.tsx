...
import { softwareproduct, softwareservice, states } from '@models/static/adminSlice.static';
...
export default ({ route, navigation }: INavigationProps2) => {
    ...
    const [formKeys, setFormKeys] = useState([
        ...
        "remarks", "product_type", "service_type"
    ])
    const [form, setForm] = useState({
        ...
        product_type: [], product_type_error: '',
        ...
        <View style={design.GENERIC_SCREEN_P}>
            <MainHeader
                render={[
                    ...
                    { type: "Title", title: "SOFTWARE SERVICES" },
                    { type: "Icon" },
                ]}
            />
            {isLoading ? <Loader /> :
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, backgroundColor: 'white' }}>
                    <ScrollView style={[design.GENERIC_CONTAINER_P, { backgroundColor: 'white' }]} >
                        ...
                        />
                        <Selection
                            _key={"softwareservice"}
                            label={"Select software service type"}
                            star={true}
                            placeholder={'Pick software service from options'}
                            options={softwareservice}
                        ...
                        />
                        <Selection
                            _key={"softwareproduct"}
                            label={"Select software product  type"}
                            star={true}
                            placeholder={'Pick software product from options'}
                            options={softwareproduct}
                            values={form.product_type}
                            onAction={({ item, index }) => setDataToForm('product_type', canPushNorPop([], item, 'name'))}
                            error={form.product_type_error}
                            closeAfterSelect={true}
                            multi={false}
                            showArrow={true}
                            isEditable={isEditable}
