...
import { isoservices, states } from '@models/static/adminSlice.static';
...
export default ({ route, navigation }: INavigationProps2) => {
    ...
    const [form, setForm] = useState({
        ...
        iso_service: [], iso_service_error: '',
        iso_service_validation: [
            { method: "isReq", params: { label: 'Remarks' } }
        ],
    })
            ...
            {isLoading ? <Loader /> :
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, backgroundColor: 'white' }}>
                    <ScrollView style={[design.GENERIC_CONTAINER_P, { backgroundColor: 'white' }]} >
                        ...
                        <Selection
                            _key={"isoservice"}
                            label={"Select ISO service type"}
                            star={true}
                            placeholder={'Pick ISO service from options'}
                            options={isoservices}
                            values={form.iso_service}
                            onAction={({ item, index }) => setDataToForm('iso_service', canPushNorPop([], item, 'name'))}
                            error={form.iso_service_error}
                            closeAfterSelect={true}
                            multi={false}
                            showArrow={true}
                            isEditable={isEditable}
