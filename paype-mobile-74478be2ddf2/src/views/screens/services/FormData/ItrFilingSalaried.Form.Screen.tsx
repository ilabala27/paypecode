...
import { states, trf } from '@models/static/adminSlice.static';
...
export default ({ route, navigation }: INavigationProps2) => {
    ...
    const [formKeys, setFormKeys] = useState([
        ...
        "father_name", "date_of_birth", "pan_no", "aadhar_no", "applicant_add", "pin_code", "app_email_id", "app_mobile_no", "financial_year", "bank_acc_no",
        "ifsc_code", "gross_salary", "deductions", "net_salary", "busi_turnover", "any_other_income", "rental_income", "interest_savings_bank",
        "lic", "health_insurance", "deduction_name", "deduction_amount"
    ])
    const [form, setForm] = useState({
        ...
        gross_salary: '', gross_salary_error: '',
        gross_salary_validation: [
            { method: "isReq", params: { label: 'Gross Salary' } }
        ],
        deductions: '', deductions_error: '',
        deductions_validation: [
            { method: "isReq", params: { label: 'Deductions' } }
        ],
        net_salary: '', net_salary_error: '',
        net_salary_validation: [
            { method: "isReq", params: { label: 'Net Salary' } }
        ],
        ...
        deduction_name: '', deduction_name_error: '',
        deduction_name_validation: [
            { method: "isReq", params: { label: 'Deduction Name' } }
        ],
        deduction_amount: '', deduction_amount_error: '',
        deduction_amount_validation: [
            { method: "isReq", params: { label: 'Deduction Amount' } }
        ],
        deduction: [], deduction_error: '',
        deduction_validation: [
            { method: "isReq", params: { label: 'Deduction ' } }
        ],
    })
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, backgroundColor: 'white' }}>
                <ScrollView style={[design.GENERIC_CONTAINER_P, { backgroundColor: 'white' }]} >
                    ...
                    <TextInput
                        ...
                        onChangeText={(text: string) => setDataToForm('gross_salary', text)}
                        error={form.gross_salary_error}
                        isEditable={isEditable}
                    />
                    <TextInput
                        label={"Deductions"}
                        ...
                        onChangeText={(text: string) => setDataToForm('deductions', text)}
                        error={form.deductions_error}
                        isEditable={isEditable}
                    />
                    <TextInput
                        label={"Net Salary"}
                        ...
                        onChangeText={(text: string) => setDataToForm('net_salary', text)}
                        error={form.net_salary_error}
                        isEditable={isEditable}
                    />
                    <TextInput
                    ...
                    />
                    <TextInput
                        label={"Invest on savings bank A/C"}
                        star={true}
                        placeholder={'eg xxxxxx'}
                    ...
                    <Selection
                        _key={"Deduction"}
                        label={"Select Any Deduction type"}
                        star={false}
                        placeholder={'Pick Any Deduction from options'}
                        options={trf}
                        values={form.deduction}
                        onAction={({ item, index }) => setDataToForm('deduction', canPushNorPop([], item, 'name'))}
                        error={form.deduction_error}
                        closeAfterSelect={true}
                        multi={false}
                        showArrow={true}
                        isEditable={isEditable}
                    />
                    <TextInput
                        label={"Deduction Name(If yes)"}
                        star={false}
                        placeholder={'eg xxxxxx'}
                        value={form.deduction_name}
                        onChangeText={(text: string) => setDataToForm('deduction_name', text)}
                        error={form.deduction_name_error}
                        isEditable={isEditable}
                    />
                    <TextInput
                        label={"Deduction Amount(If yes)"}
                        star={false}
                        placeholder={'eg xxxxxx'}
                        value={form.deduction_amount}
                        onChangeText={(text: string) => setDataToForm('deduction_amount', text)}
                        error={form.deduction_amount_error}
                        isEditable={isEditable}
                    />
        ...
        <View style={design.GENERIC_SCREEN_P}>
            <MainHeader
                render={[
                    ...
                    { type: "Title", title: "ITR FILING SALARIED" },
                    { type: "Icon" },
                ]}
            />
            <TabWithSlider
