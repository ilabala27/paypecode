...
import { digitalmarketing, states } from '@models/static/adminSlice.static';
...
export default ({ route, navigation }: INavigationProps2) => {
    ...
    const [form, setForm] = useState({
        ...
        digital_marketing: [], digital_marketing_error: '',
        digital_marketing_validation: [
            { method: "isReq", params: { label: 'Remarks' } }
        ],
    })
        ...
        <View style={design.GENERIC_SCREEN_P}>
            <MainHeader
                render={[
                    ...
                    { type: "Title", title: "DIGITAL MARKETING" },
                    { type: "Icon" },
                ]}
            />
            {isLoading ? <Loader /> :
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, backgroundColor: 'white' }}>
                    <ScrollView style={[design.GENERIC_CONTAINER_P, { backgroundColor: 'white' }]} >
                        ...
                        />
                        <Selection
                            _key={"digitalmarketing"}
                            label={"Select service type"}
                            star={true}
                            placeholder={'Pick service from options'}
                            options={digitalmarketing}
                            values={form.digital_marketing}
                            onAction={({ item, index }) => setDataToForm('digital_marketing', canPushNorPop([], item, 'name'))}
                            error={form.digital_marketing_error}
                            closeAfterSelect={true}
                            multi={false}
                            showArrow={true}
                            isEditable={isEditable}
