...
export const IsStringOptional = ({ name, description = "", ...rest }: commonType) => {
    return applyDecorators(
        IsString({ message: `${name} must be string`, ...rest, ...rest.IsString }),
        IsOptional({ ...rest, ...rest.IsOptional }),
        ApiProperty({ description, required: false, ...rest, ...rest.ApiProperty })
    )
}
...
export const IsStringRequired = ({ name, description = "", ...rest }: commonType) => {
    return applyDecorators(
        IsString({ message: `${name} must be string`, ...rest, ...rest.IsString }),
        IsNotEmpty({ message: `${name} is required`, ...rest, ...rest.IsNotEmpty }),
        ApiProperty({ description, required: false, ...rest, ...rest.ApiProperty })
    )
}
