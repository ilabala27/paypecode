...
@Injectable()
export class CrWalletCreditService {
  constructor(
    private readonly cipherService: CipherService,
    private dataSource: DataSource,
  ) { }

  private async creditWallet(manager: EntityManager, crwa_id: string, currentCredit: number, newCredit: number) {
    try {
      const actualCredit = +(parseFloat(currentCredit + '') + parseFloat(newCredit + '')).toFixed(8)
  ...
  public async creditService(cipher: cipherDto) {
    ...
    try {
      ...
      await this.creditWallet(manager, wallet.crwa_id, wallet.crwa_wallet_credit, cwtr_transaction_amount)
